...
export default {
  install (Vue, options) {
    ...
    Vue.prototype.getTicketByAxios = function () {
      let trainDate = this.$store.state.travelDate.getFullYear() + '-' + (Array(2).join('0') + (this.$store.state.travelDate.getMonth() + 1)).slice(-2) + '-' + (Array(2).join('0') + (this.$store.state.travelDate.getDate())).slice(-2)
      let fromStation = this.$store.state.startPlace.stationId
      let toStation = this.$store.state.endPlace.stationId
